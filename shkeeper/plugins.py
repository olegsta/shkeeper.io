"""Optional paid feature plugins (e.g. Multiskipper admin UI).

Plugins are discovered by package name. Set SHKEEPER_PLUGINS to a
comma-separated list (default: shkeeper_multiskipper). Missing packages
...
DEFAULT_PLUGINS = ("shkeeper_multiskipper",)
...
def has_multiskipper() -> bool:
    return is_loaded("shkeeper_multiskipper")
...
def load_plugins(app) -> dict[str, object]:
    ...
    def inject_plugins():
        return {
            "has_multiskipper": has_multiskipper(),
            "shkeeper_plugins": list(_loaded),
        }
